...
// Cek apakah jam sekarang dalam range reply bot (10:00 - 23:00) - WITA Timezone
function isBotReplyTime() {
  const now = moment().tz('Asia/Makassar').hour(); // WITA timezone
  return now >= 10 && now < 23; // 10:00 - 22:59
}
...
function getReplyTimeRemaining() {
  ...
  if (currentHour >= 10 && currentHour < 23) {
    ...
    // Diluar jam reply, hitung sisa sampai jam 10:00 besok
    let tomorrow10 = moment().tz('Asia/Makassar').hour(10).minute(0).second(0);
    if (currentHour >= 23) {
      tomorrow10.add(1, 'day');
    } else if (currentHour < 10) {
      // already today
    }
    
    const duration = moment.duration(tomorrow10.diff(now));
    ...
    return {
      isActive: false,
      hoursRemaining: timeStr.trim(),
      nextStartTime: tomorrow10.format('HH:mm:ss')
    };
  }
}
